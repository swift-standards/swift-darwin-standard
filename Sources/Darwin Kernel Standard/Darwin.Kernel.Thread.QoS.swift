...
extension Darwin_Standard_Core.Darwin.Kernel.Thread.QoS {
    ...
    ///
    /// - Parameter priority: A raw task-priority value, e.g.
...
extension Darwin_Standard_Core.Darwin.Kernel.Thread.QoS {
    ...
    /// not an override-eligible class (e.g. `unspecified`). Internal so the C
    ...
    /// (e.g. `unspecified`), `body` runs with no adjustment. Otherwise the call
